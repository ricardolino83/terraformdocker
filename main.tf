...
resource "aws_instance" "ec2_instance" {
  ...
  user_data = <<-EOF
              ...
              sudo mkdir -p challenge-day2/backend && cd challenge-day2/backend

              # Baixar o arquivo cloudmart-backend.zip
              sudo wget https://github.com/ricardolino83/terraformdocker/blob/6508e0a8676759b4de9f97ba072201b4f4e881d2/cloudmart-backend.zip

              # Descompactando o backend do sistema
              sudo unzip cloudmart-backend.zip
              ...
              sudo docker build -t cloudmart-backend .
              sudo docker run -d -p 5000:5000 --env-file .env cloudmart-backend

              cd ..

              # Criar pasta e baixar o código-fonte
              sudo mkdir frontend && cd frontend
              sudo wget https://github.com/ricardolino83/terraformdocker/blob/6508e0a8676759b4de9f97ba072201b4f4e881d2/cloudmart-frontend.zip
              sudo unzip cloudmart-frontend.zip
              ...
              #Construir e executar a imagem Docker:
              sudo docker build -t cloudmart-frontend .
              sudo docker run -d -p 5001:5001 cloudmart-frontend
              EOF
